...
test("serves the browser app from the root route", async () => {
  await withServer({ env: { OPENAI_API_KEY: "sk-test" } }, async (baseUrl) => {
    ...
    assert.match(body, /Audit microphone capture of inbound/);
    assert.match(body, /Start BlackHole 2ch live monitor/);
    assert.match(body, /Show macOS default output/);
    assert.match(body, /BlackHole 2ch live/);
...
test("serves browser app code that connects to translation over WebRTC", async () => {
  await withServer({ env: { OPENAI_API_KEY: "sk-test" } }, async (baseUrl) => {
    ...
    assert.match(body, /auditOutboundMicCapture/);
    assert.match(body, /testOutboundButton/);
    assert.match(body, /auditLeakButton/);
    assert.match(body, /auditMicCaptureButton/);
    assert.match(body, /audit\.result/);
    assert.match(body, /audit\.mic\.result/);
    assert.match(body, /LEAK DETECTED/);
    assert.match(body, /MIC LEAK DETECTED/);
    assert.match(body, /startBh2chMonitor/);
    assert.match(body, /logSystemDefaultOutput/);
